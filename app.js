...
function handleChequeFormSubmit(e) {
    ...
    if (filCurrencyCodes.includes(currency)) {
        ...
        finalPiasters = Math.floor(finalPiasters / 10); // Keep only first 2 digits
    }
...
function renderChequeList(searchTerm = '') {
    ...
    filteredCheques.forEach(cheque => {
        ...
        const amount = cheque.amount_piasters ? 
            `${cheque.amount_dinars}.${cheque.amount_piasters} ${cheque.currency}` :
            `${cheque.amount_dinars} ${cheque.currency}`;
